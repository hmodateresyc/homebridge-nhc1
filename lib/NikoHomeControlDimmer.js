...
class NikoHomeControlDimmer {
    ...
    getValue(callback) {
        this.platform.log("Get " + this.accessory.displayName + " Dimmer -> " + this.value);
        callback(null, this.value > 0);
    }

    setValue(value, callback) {
        if (value === true && this.value === 0) {
            this.platform.niko.executeActions(this.id, 100);
        } else if (value === false && this.value > 0) {
            this.platform.niko.executeActions(this.id, 0);
        }
        callback();
    }
    ...
    getBrightness(callback) {
        this.platform.log("Get Brightness " + this.accessory.displayName + " Dimmer -> " + this.value);
        callback(null, this.value);
    }

    setBrightness(value, callback) {
        this.value = value;
        this.platform.niko.executeActions(this.id, value);
        this.platform.log("Set Brightness " + this.accessory.displayName + " Dimmer -> " + this.value);
        callback();
    }
    ...
    changeValue(oldValue, newValue) {
        ...
        if (this.value !== value) {
            this.value = value;
            this.platform.log("Change " + this.accessory.displayName + " Dimmer -> " + this.value);
            this.accessory.getService?.(this.accessory.displayName)?.getCharacteristic(NikoHomeControlDimmer.Characteristic.Brightness)?.updateValue(this.value);
