...
class NikoHomeControlBlind {
    ...
    setTarget(value, callback) {
        ...
        const actionValue = (this.accessory.target < this.accessory.position) ? 254 : 255;
    ...
    changeTarget(oldValue, newValue) {
        ...
        if (oldValue === null && this.running === false) {
            ...
            this.accessory.state = 2;
            this.accessory.getService(this.accessory.displayName).getCharacteristic(NikoHomeControlBlind.Characteristic.CurrentPosition).updateValue(this.accessory.position);
            this.accessory.getService(this.accessory.displayName).getCharacteristic(NikoHomeControlBlind.Characteristic.TargetPosition).updateValue(this.accessory.target);
            this.accessory.getService(this.accessory.displayName).getCharacteristic(NikoHomeControlBlind.Characteristic.PositionState).updateValue(this.accessory.state);
